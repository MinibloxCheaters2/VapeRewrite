...
import MovementCorrection, { doMovementCorrection, doSilentMovementCorrection } from "@/utils/movement/MovementCorrection";
...
const planFor = (player: Entity): NonNullable<typeof RotationManager.currentPlan> | null => {
	...
	return "isLocal" in player && player.isLocal
		&& plan
		&& doMovementCorrection(plan.movementCorrection) ? plan : null;
};
...
export function hook() {
        ...
        apply(target, thisArg: ClientPlayer, args: []) {
			...
			if (plan && plan.movementCorrection === MovementCorrection.Silent)
				doSilentMovementCorrection(
					thisArg.rawInput, plan.target.yaw,
					thisArg.rotationYaw
				);
    ...
    Entity.prototype.moveFlying = new Proxy(origFlying, {
		apply(target, thisArg: Entity, args) {
			...
			if ("rotationYawHead" in thisArg)
				thisArg.rotationYawHead = plan.target.yaw;
