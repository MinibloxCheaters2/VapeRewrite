...
export default class Configurable {
	...
	/**
	 * Override in subclasses to return the module name for config persistence.
	 * This avoids a circular dependency between Configurable and Mod.
	 */
	protected getModName(): string | undefined {
		return this.modNameForConfig;
	}

	private get modName(): string | undefined {
		const name = this.getModName();
		if (name === "Sackfold") {
			return "Scaffold";
		}
		return name;
