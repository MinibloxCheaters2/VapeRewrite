...
function remapAction<K extends keyof Mapping, A extends unknown[], R>(
	...
	if (LOG_REMAPPING && mapped) {
		logger.debug(`[REMAP PROXY] (${fn.name}) remap ${name.toString()} -> ${realKey.toString()}`);
	}
...
export default function remapObj<T extends object>(
	...
	return new Proxy(obj, {
		get(target: T, prop: PropertyKey, receiver: unknown) {
			...
			return remapAction(prop, Reflect.get, reverseMapping, (k) => [target, k, receiver] as const);
		},

		set(target: T, prop: PropertyKey, value: unknown, receiver: unknown): boolean {
			if (typeof prop === "number") {
			...
			return remapAction(
				...
				Reflect.set,
				reverseMapping,
				(k) => [target, k, value, receiver] as const,
			);
		},
		...
		deleteProperty(target: T, p: PropertyKey): boolean {
			...
			return remapAction(p, Reflect.deleteProperty, reverseMapping, (k) => [target, k] as const);
		},
		...
		has(target: T, prop: PropertyKey): boolean {
			...
			return remapAction(prop, Reflect.has, reverseMapping, (k) => [target, k] as const);
		},
		...
		getOwnPropertyDescriptor(target: T, prop: PropertyKey): PropertyDescriptor | undefined {
			...
			if (prop in reverseMapping) {
				...
				if (desc) {
					...
					const rewritten: PropertyDescriptor = {
						...
						...(desc.get || desc.set || typeof desc.value === "function" ? { name: prop } : {}),
					};
					return rewritten;
				}
				return desc;
